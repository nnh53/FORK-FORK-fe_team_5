import { type FC, forwardRef, type ReactNode, useEffect, useImperativeHandle, useMemo, useRef } from "react";
...
// Safe random number generator for graphics - not cryptographic use
function generateGraphicsRandom(max: number): number {
  const randomValue = Math.random(); // eslint-disable-line sonarjs/pseudo-random
...
function createStackedPlanesBufferGeometry(n: number, width: number, height: number, spacing: number, heightSegments: number): THREE.BufferGeometry {
  ...
  // Pre-generate random values to avoid performance issues in loops
  const randomUVOffsets = Array.from({ length: n }, () => ({
    uvXOffset: generateGraphicsRandom(300),
    uvYOffset: generateGraphicsRandom(300),
  }));

  for (let i = 0; i < n; i++) {
    const xOffset = xOffsetBase + i * (width + spacing);
    const { uvXOffset, uvYOffset } = randomUVOffsets[i];
