        ...
        const oldFormatShowtimes = await Promise.all(
          futureShowtimes.map(async (newShowtime) => {
            ...
            return {
              ...
              date: showDate.toICTISOString().split("T")[0],
    ...
    navigate(`/movie/${movie.id}`, {
      state: {
        movie: {
          ...
          startShowingDate: new Date().toICTISOString().split("T")[0],
          endShowingDate: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toICTISOString().split("T")[0],
          status: "active",
          version: "2D",
          showtimes: [],
