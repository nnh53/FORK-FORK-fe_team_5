...
const NavItem: React.FC<{ item: NavItemData }> = ({ item }) => {
  ...
  return (
    <div className="relative group">
      <Link
        to={item.path}
        className={`navItem text-black uppercase hover:text-red-500 transition duration-300 cursor-pointer font-bold px-4 py-2 ${
        ...
        <div className="absolute left-0 mt-2 w-48 bg-amber-50 rounded-md shadow-lg py-1 z-50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 invisible group-hover:visible">
          {item.children.map((child) => (
            <Link
              key={child.path}
              to={child.path}
              className="block px-4 py-2 text-sm text-gray-900 border border-transparent hover:border-red-500 hover:text-red-500 transition duration-200"
            >
              {child.label}
            </Link>
          ))}
