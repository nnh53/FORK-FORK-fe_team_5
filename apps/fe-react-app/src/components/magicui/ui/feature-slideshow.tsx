...
const AccordionTrigger = forwardRef<HTMLButtonElement, AccordionTriggerProps>(({ children, className, ...props }, forwardedRef) => (
  <Accordion.Header className="flex">
    <Accordion.Trigger
      className={cn("group flex h-[45px] flex-1 cursor-pointer items-center justify-between p-3 text-[15px] leading-none outline-none", className)}
      {...props}
      ref={forwardedRef}
    >
      {children}
...
const AccordionContent = forwardRef<HTMLDivElement, AccordionContentProps>(({ children, className, ...props }, forwardedRef) => (
  <Accordion.Content
...
export const Feature = ({
  ...
  const renderMedia = () => {
    ...
    if (!currentItem) {
      return <div className="aspect-auto h-full w-full animate-pulse rounded-xl border border-neutral-300/50 bg-gray-200 p-1" />;
    }
    ...
    return <div className="aspect-auto h-full w-full rounded-xl border border-neutral-300/50 bg-gray-200 p-1" />;
  };
    ...
    <div ref={ref} className="w-full">
      <div className="mx-auto flex w-full max-w-7xl flex-col items-center justify-center">
        <div className="grid h-full w-full grid-cols-5 items-center gap-x-10 px-10 md:px-20">
          <div className={`col-span-2 hidden h-full w-full md:items-center lg:flex ${ltr ? "md:order-2 md:justify-end" : "justify-start"}`}>
            <Accordion.Root
                ...
                >
                  <div
                    className={cn(
                      ...
                      {
                        "bottom-0 left-0 top-0 h-full w-0.5": linePosition === "left",
                        "bottom-0 right-0 top-0 h-full w-0.5": linePosition === "right",
                        "left-0 right-0 top-0 h-0.5 w-full": linePosition === "top",
                        "bottom-0 left-0 right-0 h-0.5 w-full": linePosition === "bottom",
                      },
                    )}
                  ...
                  <AccordionTrigger className="text-left text-lg font-semibold tracking-tight">{item.title}</AccordionTrigger>
                  <AccordionContent className="text-sm font-medium">{item.content}</AccordionContent>
                </AccordionItem>
              ))}
            </Accordion.Root>
          </div>
          <div className={`col-span-5 h-[350px] min-h-[200px] w-auto lg:col-span-3 ${ltr && "md:order-1"}`}>{renderMedia()}</div>
