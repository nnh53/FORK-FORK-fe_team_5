...
import { type ChartConfig, ChartContainer, ChartTooltip, ChartTooltipContent } from "@/components/Shadcn/ui/chart";
import { queryReceiptTopMovies } from "@/services/receiptService";
import { useMemo } from "react";

export function TopMoviesRevenuePieChart() {
  ...
  return (
    <Card className="flex flex-col">
      <CardHeader className="items-center pb-0">
        <CardTitle>Top Movies Revenue</CardTitle>
      ...
      <CardContent className="flex-1 pb-0">
        <ChartContainer config={chartConfig} className="[&_.recharts-pie-label-text]:fill-foreground mx-auto aspect-square max-h-[250px] pb-0">
          <PieChart>
            ...
            <Pie data={chartData} dataKey="revenue" label nameKey="name" />
          </PieChart>
        </ChartContainer>
      </CardContent>
      <CardFooter className="flex-col gap-2 text-sm">
        ...
        <div className="text-muted-foreground leading-none">Showing top movie revenue for the current month</div>
      </CardFooter>
    </Card>
  );
}

export default TopMoviesRevenuePieChart;
